...
```{=latex}
\begin{enumerate}
    \item Exakter Titel + Autor: \texttt{tit=\char34 Exact Title\char34{} AND per=Author}
    \item Titel-Wörter + Autor: \texttt{tit=Title Words AND per=Author}
    \item Exakter Titel: \texttt{tit=\char34 Exact Title\char34}
    \item Titel-Wörter: \texttt{tit=Title Words}
\end{enumerate}
```
...
```{=latex}
\begin{enumerate}
    \item Exakter Titel + Jahr: \texttt{tit=\char34 Exact Title\char34{} AND date=Year}
    \item Titel-Wörter + Jahr: \texttt{tit=Title Words AND date=Year}
\end{enumerate}
```
...
flowchart TB
    ...
    subgraph "Suchstrategien (DNB/LoC parallel)"
    ...
    Variants -->|1 DNB oder LoC| AutoSelect[Auto-Select beste Variante]
    Variants -->|2+ Varianten| AI[KI-Fusion<br/>Ollama llama3.3:70b]

    AI --> Decision{KI-Entscheidung}
    Decision -->|Variante wählen| Normalize[String-Normalisierung]
    Decision -->|Keine passend| Reject[❌ Alle verwerfen]
...
```
<figcaption>Abbildung: Ablauf der dreistufigen Anreicherungsstrategie mit sprachbasierter Quellenpriorisierung und KI-gestützter Variantenauswahl</figcaption>
</figure>
